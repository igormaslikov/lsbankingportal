...
$Destination1_bulk=$row[3];
$Seller_Name1_bulk=$row[4];
$Box_Weight1_bulk=$row[5];
$shipment_notes1_bulk=$row[6];
$quantity_bulk= $row[7];
$box_number = $row[10];


//$data_shipment = "Shipment Key : " .$Shipment_key1_bulk;
$data_shipment  = "Shipment ID :" .$Shipmentt_ID_bulk;
$data_shipment .= "- BOX ID :" .$box_number;
$data_shipment .= "- Box Weight :" .$Box_Weight1_bulk;
$data_shipment .= " - TOTAL UNITS:" .$quantity_bulk ;
//$data_shipment .= "Shipment Notes :" .$shipment_notes1_bulk;
}

//$pdf->write2DBarcode($data_shipment, 'PDF417', 10, 10, 0, 90, $style);
//$pdf->Text(15, 100, $data_shipment);


$pdf->Text(15, 100, "SHIPMENT ID : ".$Shipmentt_ID_bulk." | BOX NUMBER: " . $box_number );
$pdf->Text(15, 105, "WEIGHT : ". $Box_Weight1_bulk. " | TOTAL UNITS : ". $quantity_bulk );
...
$month= substr($item_Expiration_Date, 5, 7);
$year= substr($item_Expiration_Date, 2, 2);
$date= substr($item_Expiration_Date, 8, 10);

$barcode_items .= ",".$item_id_type.":".$item_Identifier . ",QTY:" . $item_Quantity.","; 
if ($item_Expiration_Date == "") {
}
else {
	$barcode_items .= "EXP:".$year.$month;
}
...
$shipment_replace = str_replace(" ","",$Shipmentt_ID_bulk);
...
// CODE 128 AUTO
$pdf->write1DBarcode($shipment_replace."U".str_pad($box_number,6,"0",STR_PAD_LEFT), 'C128', '', '', '', 35, 0.8, $style, 'N');

$pdf->Ln();

$pdf->Text(65, 177, $shipment_replace."U".str_pad($box_number,6,"0",STR_PAD_LEFT));

$pdf->write2DBarcode($shipment_replace."U".str_pad($box_number,6,"0",STR_PAD_LEFT), 'PDF417', 90, 180, 300, 80, $style1);
...
$path="Barcodes/$Shipment_key1_bulk.pdf";
