...
$customer_name = '';
while ($sql_name && ($row_name = $sql_name->fetch_array())) {
...
if ($website_DL_check == 'lspaydayloans'){
...
Gracias por aplicar con Optima Financial Solutions Inc, lamentamos informarle que su aplicación ha sido declinada, usted no ha cumplido con los criterios correpondientes para extenderle un préstamo en estos momentos. Puede volver a aplicar después de 90 días.";
...
$con->query($query_update_status); 
 ...
 //$DL_overdraft_ov = 11;
if (($DL_available_balance<-200) ||  ($DL_overdraft_ov>7) || ($DL_avg_balance_last_month<-300)){

if ($website_DL_check == 'lsprestamos'){
...
$con->query("UPDATE fnd_user_profile SET application_status ='Approved Payday Loan' where email = '$email' AND application_status!='Declined' AND website = 'lspaydayloans'");
...
Please Follow the link (https://ofsca.com/loanportal/ls_software/dl_client_files/index.php?email='.$email.'&code='.$code.') to submit your documents (Photo ID, Bank Card Front/Back) to LS BANKING';
$headers = 'From: info@lspaydayloans.com';
mail($to_email,$subject,$message,$headers);
