...
$query_us = mysqli_query($con,"SELECT SUM(amount_of_loan) AS value_sum FROM tbl_title_loans ");
...
$query_le = mysqli_query($con,"SELECT SUM(loan_total_payable) AS value_sum FROM tbl_title_loans ");
...
$sql=mysqli_query($con, "select * from tbl_title_loans where sign_status= '1'"); 
$total_loan_fee="0";
...
$total_records_per_page = 200;
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `tbl_title_loans` ");
...
break;
   // echo"<br>User_Key:" .$payment;
        ...
        $result_user_totalloan = mysqli_query($con,"Select * from `tbl_title_loans` where user_fnd_id = '$user_fnd_id' ");
        $total_loans_lh = 0;
