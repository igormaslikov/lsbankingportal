...
$query_search = "SELECT * FROM tbl_title_loans";
...
$query_us = $con->query("SELECT SUM(TRY_CAST(amount_of_loan AS DECIMAL(18,2))) AS value_sum FROM tbl_title_loans ");
...
$query_le = $con->query("SELECT SUM(TRY_CAST(loan_total_payable AS DECIMAL(18,2))) AS value_sum FROM tbl_title_loans ");
...
$sql=$con->query("select * from tbl_title_loans where sign_status= '1'"); 
$total_loan_fee="0";
...
$total_records_per_page = 200;
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_title_loans ");
...
$query_search .= " order by loan_create_id asc OFFSET " . $offset . " ROWS FETCH NEXT " . $total_records_per_page . " ROWS ONLY";
...
break;
   // echo"<br>User_Key:" .$payment;
        ...
        $result_user_totalloan = $con->query("Select * from tbl_title_loans where user_fnd_id = '$user_fnd_id' ");
        $total_loans_lh = 0;
...
if ($last_payment_date=='01-01-1970')
{
  ...
  $balns_due= number_format((float)$balns_due,2);
		...
		// $envalope= "<a href='view_all_loan_notification.php?id=$id'  title='View Notifications'><span class='glyphicon glyphicon-envelope' aria-hidden='true' alt='View Notifications'></span></a>";
          ...
          $make_payment= "<a href='?id=$id'  title='Make Payment' style='color:red;'><span class='glyphicon glyphicon-usd' aria-hidden='true' alt='Make Payment'></span></a>";
		echo "<tr class=".$loan_status.">
...
<td style='text-align:center;'><a href='?id=$id'  title='View Summary' style='color:black;margin-right:4px;'><span class='glyphicon glyphicon-user' aria-hidden='true' alt='edit'></span></a>    ";
