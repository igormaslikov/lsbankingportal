...
$query_payment = $con->query("SELECT SUM(TRY_CAST(payoff_amount AS DECIMAL(18,2))) AS value_sum FROM tbl_commercial_loan_installments where loan_create_id='$loan_create_id' AND status= '0'");
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_commercial_loan_installments where loan_create_id='$loan_create_id' AND status='0'");
	$total_records = $result_count->fetch_array();
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = $con->query("select * from tbl_commercial_loan_installments where loan_create_id='$loan_create_id' AND status='0'");
