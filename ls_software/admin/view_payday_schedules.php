...
<?php include('menu.php') ;?>
 ...
 <?php
    ...
    $query_search = "SELECT * FROM `tbl_loan_schedules` ORDER BY id DESC";
    $status  = $_GET['status'];
// echo $query_search;
...
<th style='width:10%;color:black;'>Customer Name</th>
<th style='width:5%;color:black;'> Amount</th>
<th style='width:10%;color:black;'>Scheduled Date</th>
<th style='width:10%;color:black;'>Processing Time</th>
<th style='width:5%;color:black;'>Transaction ID</th>
<th style='width:15%;color:black;'>Card</th>
<th style='width:20%;color:black;'>Status</th>
</tr>
</thead>
<tbody>
<?php
...
$loann_id=$row_fnd_idd['loan_id'];
$user_fnd_idd=$row_fnd_idd['user_fnd_id'];

$sql_user=mysqli_query($con, "select * from tbl_loan where loan_create_id= '$loann_id'"); 

while($row_user = mysqli_fetch_array($sql_user)) {

$user_fnd_idd=$row_user['user_fnd_id'];
...
$gravatar =  "http://profiles.google.com/s2/photos/profile/". $row['email']."?sz=";
		echo "<tr ".$string_red_rejected. " ".$bold_app. ">
			  ...
			  <td>".$row_fnd_idd['loan_id']."</td>
			  <td>".$user_fnd_idd." </td>
			  <td>".$name_customer."</td>
	 		  <td>$".$row_fnd_idd['amount']."</td>
	 		  <td>".$row_fnd_idd['payment_date']." ".$row_fnd_idd['schedule_time']."</td>
	 		  <td>".$row_fnd_idd['payment_time']." ".$row_fnd_idd['payment_date_done']."</td>
	 		  <td>".$row_fnd_idd['transaction_id']."</td>
	 		  <td>".$row_fnd_idd['card']."</td>
	 		  <td>".$row_fnd_idd['payment_report']."</td>
