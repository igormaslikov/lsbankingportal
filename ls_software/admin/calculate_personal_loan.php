...
<input type="submit" name="button5" value="Calculate Payment"><br><br>
...
echo '<tr style="background-color: #F5E09E;"><th>#</th><th>PAYMENT</th><th>INTEREST</th><th>PRINCIPAL</th><th>BALANCE</th></tr>';
  include 'dbconfig.php';
    ...
    $loan_create_id=$_GET['loan_create_id'];
    $installment_plan=$_GET['installment_plan'];
    $payment_date=$_GET['contract_date'];
...
$payment_date_weekly= date( "Y-m-d", strtotime( "$dt +30 day" ) );
   ...
   $payment_p = 
     $query_install1  = "INSERT INTO `tbl_personal_loan_installments`(`loan_create_id`, `payment`, `interest`, `principal`, `balance`, `payment_date`) VALUES ('$loan_create_id','$payment_p','$interest_p','$principal_p','$balance_p','$payment_date_weekly')";
...
<?php
     ...
     $state=$_GET['state'];
echo
"<a href = 'personal_initial_setup.php?fnd_id=$fnd_idd&interest=$interest&bg_id=$source&loan_create_id=$loan_create_id&principal_amount=$principal_amount&loan_interest=$loan_interest&years=$years&late_fee=$late_fee&contract_fee=$origination&installment_plan=$installment_plan&total_payments=$total_payments&contract_date=$contract_date&payment_date=$payment_date&state=$state'><button name='' type='submit' class='btn btn-danger' style='background-image: linear-gradient(to bottom,#1E90FF 0,#1E90FF 100%);color: #fff;background-color: #1E90FF;border-color: #1E90FF;'>Create Installment Loan</button></a>"; 
