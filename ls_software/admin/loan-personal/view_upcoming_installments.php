...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `tbl_personal_loan_installments` where status='0'");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = mysqli_query($con, "select * from tbl_personal_loan_installments where status='0' order by payment_date ASC");
...
$result_query = mysqli_query($con, "select * from tbl_personal_loans where loan_create_id='$loan_create_id'");
    while($row_payfre = mysqli_fetch_array($result_query)){
        $p_loan_id= $row_payfre['p_loan_id'];
...
if($status=='1')
{
 ...
 $a="<a href='add_new_transaction.php?intallment_id=$intallment_id&id=$p_loan_id' class='disabled'>Pay Now</a>";
}

else
{
    $installment_status="Unpaid";
    $a="<a href='add_new_transaction.php?intallment_id=$intallment_id&id=$p_loan_id'>Pay Now</a>";
}
