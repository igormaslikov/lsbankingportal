    ...
    if ($delete_allowed==1)
{
    ...
    $date=date('Y-m-d');
$del_reason ="Loan ID $loan_create_id is deleted,".$_POST['del_reason']." and User ID is $user_fnd_id.";

    $query = "DELETE FROM tbl_personal_loans WHERE p_loan_id = '$id'";
    $result = $con->query($query);
    
    $query_transaction = "DELETE FROM personal_loan_transaction WHERE loan_id = '$id'";
    $result_transaction = $con->query($query_transaction);
    
    $query_installments = "DELETE FROM tbl_personal_loan_installments WHERE loan_create_id = '$loan_create_id'";
    $result_installments = $con->query($query_installments);
