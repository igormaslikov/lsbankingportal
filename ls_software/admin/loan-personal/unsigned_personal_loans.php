...
if(isset($_GET['signed_loan'])) {
    ...
    $con->query("UPDATE tbl_personal_loans SET sign_status='1' WHERE p_loan_id = '$signed_loan_id'");
}  
if(isset($_GET['delete_loan'])) {
    $signed_loan_id = $_GET['delete_loan'];
    $con->query("DELETE FROM tbl_personal_loans WHERE p_loan_id = '$signed_loan_id'");
}  
$query_search = "SELECT * FROM tbl_personal_loans where  where sign_status= '0'";
...
$query_us = $con->query("SELECT SUM(TRY_CAST(amount_of_loan AS DECIMAL(18,2))) AS value_sum FROM tbl_personal_loans where sign_status= '0'");
...
$query_le = $con->query("SELECT SUM(TRY_CAST(loan_total_payable AS DECIMAL(18,2))) AS value_sum FROM tbl_personal_loans where sign_status= '0'");
while ($query_le && ($row_le = $query_le->fetch_array())){
    $pay_off = $row_le['value_sum'];
break;
}

$query_trns = $con->query("SELECT SUM(TRY_CAST(payoff_amount AS DECIMAL(18,2))) AS value_sum FROM personal_loan_transaction ");
...
$avg = number_format((float)$avg_amount, 2, '.', '');
 ...
 $sql_fees="SELECT loan_create_id, COUNT(*) FROM personal_loan_transaction GROUP BY loan_create_id;";
...
$sql=$con->query("select * from tbl_personal_loans where sign_status= '0'"); 
$total_loan_fee="0";
...
$query_payment_fee = $con->query("SELECT SUM(TRY_CAST(payoff_amount AS DECIMAL(18,2))) AS value_summ FROM personal_loan_transaction where loan_id= '$loan_id_fee'");
...
<div align="right" style="padding:30px;background-color: #F5E09E;color: white">
     <a href="home.php"> <button name="btn-submit" type="submit" class="btn btn-danger" style="background-color: #1E90FF;color: white;border-color: #1E90FF;">Signed Personal Loans</button></a>
...
$total_records_per_page = 200;
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_personal_loans where sign_status= '0'");
...
$query_search = "SELECT * FROM tbl_personal_loans  where sign_status= '0'";
...
$query_payment = $con->query("SELECT SUM(TRY_CAST(payoff_amount AS DECIMAL(18,2))) AS value_sum FROM personal_loan_transaction where loan_id= '$loan_id_calculation'");
...
break;
   // echo"<br>User_Key:" .$payment;
        ...
        $result_user_totalloan = $con->query("Select * from tbl_personal_loans where user_fnd_id = '$user_fnd_id' AND sign_status = '0' ");
        $total_loans_lh = 0;
...
<td style='text-align:center;'><a href='loan_summary.php?id=$id'  title='View Summary' style='color:black;margin-right:4px;'><span class='glyphicon glyphicon-user' aria-hidden='true' alt='edit'></span></a>    ";
    ...
    <a class='remove-box' href='delete_loan.php?loan_id=<?php echo $id;?>&fnd_id=<?php echo $user_fnd_id;?>'  title='Delete Loan'><span class='glyphicon glyphicon-remove' aria-hidden='true' alt='$alt;'></span>  <span style = 'color:green; text-align:left'></span></a>
    <?php 
