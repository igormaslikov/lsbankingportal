...
if(isset($_GET['signed_loan'])) {
    ...
    mysqli_query($con,"UPDATE `tbl_personal_loans` SET `sign_status`='1' WHERE `p_loan_id` = '$signed_loan_id'");
}  
if(isset($_GET['delete_loan'])) {
    $signed_loan_id = $_GET['delete_loan'];
    mysqli_query($con,"DELETE FROM `tbl_personal_loans` WHERE `p_loan_id` = '$signed_loan_id'");
}  
...
$query_us = mysqli_query($con,"SELECT SUM(amount_of_loan) AS value_sum FROM tbl_personal_loans where sign_status= '0'");
...
$query_le = mysqli_query($con,"SELECT SUM(loan_total_payable) AS value_sum FROM tbl_personal_loans where sign_status= '0'");
while ($row_le=mysqli_fetch_array($query_le)){
    $pay_off = $row_le['value_sum'];
break;
}

$query_trns = mysqli_query($con,"SELECT SUM(payoff_amount) AS value_sum FROM personal_loan_transaction ");
...
$sql=mysqli_query($con, "select * from tbl_personal_loans where sign_status= '0'"); 
$total_loan_fee="0";
...
$query_payment_fee = mysqli_query($con,"SELECT SUM(payoff_amount) AS value_summ FROM personal_loan_transaction where loan_id= '$loan_id_fee'");
...
$total_records_per_page = 200;
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `tbl_personal_loans` where sign_status= '0'");
...
$query_payment = mysqli_query($con,"SELECT SUM(payoff_amount) AS value_sum FROM personal_loan_transaction where loan_id= '$loan_id_calculation'");
...
break;
   // echo"<br>User_Key:" .$payment;
        ...
        $result_user_totalloan = mysqli_query($con,"Select * from `tbl_personal_loans` where user_fnd_id = '$user_fnd_id' AND sign_status = '0' ");
        $total_loans_lh = 0;
