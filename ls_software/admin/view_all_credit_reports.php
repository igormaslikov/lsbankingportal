...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_credit_report ORDER BY credit_report_key DESC");
	$total_records = $result_count->fetch_array();
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = $con->query("SELECT * FROM tbl_credit_report ORDER BY credit_report_key DESC OFFSET $offset ROWS FETCH NEXT $total_records_per_page ROWS ONLY");
    while($row = $result->fetch_array()){
		...
		$new_creationDate = date("m-d-y H:i:s", strtotime($creation_date));
		 ...
		 $access_level_name = "";
		 $sql_access=$con->query("select * from fnd_user_profile where user_fnd_id= '$app_id'"); 
