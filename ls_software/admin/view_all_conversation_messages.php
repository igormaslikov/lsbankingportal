...
<th style='width:6%;color:black;'>User ID</th>
<th style='width:11%;color:black;'>Customer Name</th>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_conversation ORDER BY date DESC");
	$total_records = $result_count->fetch_array();
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = $con->query("SELECT * FROM tbl_conversation ORDER BY date DESC OFFSET $offset ROWS FETCH NEXT $total_records_per_page ROWS ONLY");
    while($row = $result->fetch_array()){
		...
		$incoming_read = $row['incoming_read'];
...
$style_unread = "";
if($incoming_read == 0){
    $style_unread = "style='font-weight:800'";
}


		echo "<tr $style_unread>
			  ...
			  <td>".$customer_name."</td>
	 		  ...
	 		  <td>".$message." $deliverd_sms</td>
		   	  ...
		   	  <td><a href='edit_customer.php?id=$user_fnd_id' title='View All Messages'><button style='background-color: #1E90FF;color: white;'>View Conversation</button></a></td>
