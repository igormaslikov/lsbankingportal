...
$query = "SELECT * FROM fnd_user_profile WHERE date_time_current < DATEADD(MINUTE, -15, GETDATE()) AND application_status = 'Decision Logic Completed'  AND decision_logic_status = '1' AND (website = 'lsprestamos' OR website = 'lsbanking_pl') AND experian_api_limit < 1 ORDER By user_fnd_id DESC OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY";
...
$score= $quoteJson->creditProfile[0]->riskModel[0]->score;  
 ...
 $score = str_replace(" ","",$score);
    ...
    if($score>500)
    {
          $con->query("UPDATE fnd_user_profile SET application_status='Final Review For Personal Loan' where user_fnd_id = '$application_id'");
          $con->query("UPDATE fnd_user_profile SET experian_credit_score='$score' where user_fnd_id = '$application_id'");
          $con->query("UPDATE fnd_user_profile SET  experian_api_limit='1'  where user_fnd_id = '$application_id'");
