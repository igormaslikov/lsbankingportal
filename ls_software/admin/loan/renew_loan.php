...
if (!isset($_POST['btn-submit']) && isset($_REQUEST['card_number'])) {
  ...
  $sql_bank_detail = $con->query("select distinct card_exp_date, cvv_number, type_of_card, bank_name from loan_initial_banking where user_fnd_id = (SELECT user_fnd_id from tbl_loan WHERE loan_id = '$loan_id') and card_number='$card_number'");
  // $type_of_card = '';
  // $renew_year = '';
  // $rerenew_month = '';
  // $cvv_number = '';
...
?>
  ...
  $sql_bank_detail = $con->query("select * from loan_initial_banking where user_fnd_id = '$user_fnd_id' AND loan_id= '$loan_create_id'");
      ...
      <option value="Master Card" <?php if ($type_of_card == 'Master Card') {
                  ...
                  <div class="col-lg-6">
                    ...
                    <select name="card_number" id="card_number" class="form-control" value="" required>
                      <?php
                      $sql_card_details = $con->query("SELECT DISTINCT card_number FROM loan_initial_banking WHERE user_fnd_id = '$user_fnd_id'");
