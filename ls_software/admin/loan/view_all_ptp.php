...
<div class="col-lg-3"><p>Loan Amount: <b style="color:red"> <?php echo $val.$amount_loan.$variable;?></b></p></div>
...
$result_status = mysqli_query($con,"SELECT * FROM tbl_ptp where loan_id= '$loan_create_id' ORDER BY ptp_id desc");

echo '<br><table style="width:100%;padding:10px" class="table table-striped table-bordered">'."
<tr>
<th style='width:70%'>PTP Notes Detail</th>
...
$ptp_note_db =$row_status['notes_ptp'] ;

	


$pt_creation_date=	$row_status['creation_date'];
$timestamp = strtotime($pt_creation_date);
$newptp_creation_date= date("m-d-Y", $timestamp);
	
echo "<tr>";
echo "<td>" .$ptp_note_db. "</td>";
echo "<td>".$final_activity_by_user."</td>";
echo "<td>" . $newptp_creation_date . "</td>";
echo "</tr>";
}
echo "</table><br>";
