...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM loan_initial_banking where user_fnd_id = '$user_fnd_id'");
	$total_records = $result_count->fetch_array();
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

   $sql_loan=$con->query("select * from loan_initial_banking where user_fnd_id = '$user_fnd_id'  GROUP BY card_number, card_exp_date ORDER BY initial_id DESC"); 
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM  tbl_bank_cards where user_fnd_id = '$user_fnd_id'");
	$total_records = $result_count->fetch_array();
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

   $sql_loan=$con->query("select * from  tbl_bank_cards where user_fnd_id = '$user_fnd_id'"); 
