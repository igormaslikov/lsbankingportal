...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_payment_method where user_fnd_id='$user_fnd_id'");
	$total_records = $result_count->fetch_array();
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = $con->query("select * from tbl_payment_method where user_fnd_id='$user_fnd_id'");
    while($row = $result->fetch_array()){
		...
		echo "<tr>
			  ...
			  <td>".$account_number."</td>
	 		  <td>".$cvv."</td>
	 		  <td>".$created_by."</td>
	 		  <td><a href='edit_payment_method.php?m_id=$id_payment&id=$id'>Edit</a> <a href='delete_payment_method.php?id=$id_payment&loan_id=$id'>Delete</a></td>
