...
$query_search = "SELECT * FROM tbl_loan where loan_status='Paid' AND last_payment_date >='$date_last_7days'";
...
$query_us = $con->query("SELECT SUM(TRY_CAST(amount_of_loan AS DECIMAL(18,2))) AS value_sum FROM tbl_loan where loan_status='Paid' AND last_payment_date >='$date_last_7days'");
...
$query_le = $con->query("SELECT SUM(TRY_CAST(loan_total_payable AS DECIMAL(18,2))) AS value_sum FROM tbl_loan where loan_status='Paid' AND last_payment_date >='$date_last_7days'");
...
$sql=$con->query("select * from tbl_loan where loan_status='Paid' AND last_payment_date >='$date_last_7days'"); 
$total_loan_fee="0";
...
<option value="" <?php if($_GET['state']=='All'){ echo 'selected';}?>>Select</option>
...
$total_records_per_page = 200;
	...
	$result_count = $con->query("SELECT COUNT(*) As total_records FROM tbl_loan where loan_status='Paid' AND last_payment_date >='$date_last_7days'");
...
break;
   // echo"<br>User_Key:" .$payment;
        ...
        $result_user_totalloan = $con->query("Select * from tbl_loan where user_fnd_id = '$user_fnd_id' AND loan_status='Paid' AND last_payment_date >='$date_last_7days'");
        $total_loans_lh = 0;
