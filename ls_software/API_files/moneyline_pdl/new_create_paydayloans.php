...
echo "$emp_zip<br>";
echo "$emp_amount<br>";
echo "$direct_deposit<br>";
echo "$emp_payfre<br>";
echo "$last_paydate<br>";
echo "$nxt_paydate<br>";


if ($customer_state == 'AZ') {
  $web_site = "Payday Arizona";
} else if ($customer_state == 'NV') {
  $web_site = "Payday Nevada";
} else {
  $web_site = "Payday $customer_state";
}
...
$emp_state = str_replace("-", " ", "$emp_state");
...
$sql_fnd = $con->query("select * from fnd_user_profile where mobile_number = '$customer_tel'");
...
if ($rowcount_funded > 0) {
  ...
  if ($creation_date < $date_duplicate) {
    ...
    $loan_transaction_id = "";
    $con->query("UPDATE fnd_user_profile SET application_status='New Application', application_date='$to_date_filter', loan_type='$type_loan' user_fnd_id ='$user_fnd_iddd'");
...
';
  ...
  $sql_fnd_11 = $con->query("select * from fnd_user_profile where email = '$customer_email'");
  ...
  $query_employment_entry = "INSERT INTO source_income(user_fnd_id,employer_name,work_phone_no,net_check_amount,direct_deposit,pay_period,last_pay_date,next_pay_date,address_b,city_b,state_b,zip_b) VALUES ('$user_fnd_id','$emp_name','$emp_phone','$emp_amount','$direct_deposit','$emp_payfre','$last_paydate','$nxt_paydate','$emp_address','$emp_city','$emp_state','$emp_zip')";
