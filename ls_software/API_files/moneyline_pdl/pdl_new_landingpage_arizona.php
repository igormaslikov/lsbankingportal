...
{
    $web_site="Installment Arizona";
}

else if ($customer_state=='NV')
{
    $web_site="Installment Nevada";
}

else{
    $web_site="Installment $customer_state";
}
...
//echo "select * from fnd_user_profile where email = '$customer_email' AND mobile_number = '$customer_tel'";
         $date_duplicate= date('Y-m-d', strtotime('-7 day'));
              ...
              $sql_fnd=$con->query("select * from fnd_user_profile where email = '$customer_email' AND mobile_number = '$customer_tel' AND date_of_birth='$dobb'"); 
...
$con->query("UPDATE fnd_user_profile SET application_status='New Application', application_date='$to_date_filter', loan_type='$type_loan' where user_fnd_id ='$user_fnd_iddd'");
