...
//echo $type_id;
 ...
 $query_userid3 = $con->query("Select * from fnd_user_profile where user_fnd_id ='$fnd_idd'");
...
$sql_count_ca = $con->query("SELECT * FROM tbl_loan where state='$state_insrt' AND loan_create_id!='' order by loan_create_id ASC");
while ($row_count_ca=$sql_count_ca->fetch_array()){
...
if ($state_insrt=="CA")
{
 $query3 = $con->query("Select loan_fee from tbl_loan_setting_nv where loan_amount ='$amount_loan' AND state='$state_insrt'");
 ...
 if ($state_insrt=="NV")
{
 $query33 = $con->query("Select loan_fee from tbl_loan_setting_nv where loan_amount ='$amount_loan' AND state='$state_insrt'");
